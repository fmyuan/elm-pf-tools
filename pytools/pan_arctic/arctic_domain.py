...
def domain_ncwrite(elmdomain_data, WRITE2D=True, ncfile='domain.nc', coord_system=True):
    ...
    if not WRITE2D:
        ...
        if coord_system and 'X_axis' in elmdomain_data.keys():
            ...
            grid_yid_arr = grid_yid_arr[TLND_MASK]
        lon_arr = lon_arr[TLND_MASK]
        lat_arr = lat_arr[TLND_MASK]
        xv_arr  = xv_arr[TLND_MASK,]
        yv_arr  = yv_arr[TLND_MASK,]
        area_arr = area_arr[TLND_MASK]
        landfrac_arr = landfrac_arr[TLND_MASK]
                ...
                if dname in indx_dim:
                    if indx_dim_flatten!='':
                        ...
                        if indx_dim.index(dname)==len(indx_dim)-1: 
                            if reordered_box:
                                dim_len = npwhere_mask.size
                            else:
                                dim_len = sum(npwhere_mask>0)
                            
                        elif (dname=='nj' and indx_dim_flatten=='ni'):
                            dim_len = 1
                        else: 
                            continue
                    else:
                    ...
                    for i in range(len(src_vdim)):
                        if i==0:
                            ...
                            if (i in ix and i==ix_mask+ix[0]) or \
                                ...
                                if indx_dim_flatten!='' and not reordered_box:
...
def refine_surfdata(outdir='./', \
                    ...
                    SUBDOMAIN_REORDER = False, KEEP_DUPLICATED = False, \
    ...
    for ncfile in allncfiles:
        ...
        fout = ncdata_subsetbynpwhereindex(idx_box, npwhere_mask=newmask_box, npwhere_frac=newfrac_box, \
                                    ...
                                    reordered_box=SUBDOMAIN_REORDER or KEEP_DUPLICATED, \
...
if __name__ == '__main__':
    ...
    set_e3sm_input('/Users/f9y/project_e3sm/e3sm_inputdata')
    ''''''

    '''
    # extrac pt or list of pts of domain/surfdata - changing resolutions and grid ordering
    ...
    os.system('mv domain.lnd.r0125_IcoswISC30E3r5.250918.nc_remasked domain.lnd.r0125_IcoswISC30E3r5.250918_remasked.nc')

    allout = refine_surfdata( \
                    lnd_domain_file='domain.lnd.r0125_IcoswISC30E3r5.250918.nc', \
                    fsurdat='surfdata_0.125x0.125_simyr1850_c250910_TOP.nc', \
                    flanduse_timeseries=None, \
                    userdomain='domain.lnd.r0125_IcoswISC30E3r5.250918_remasked.nc', \
                    SUBDOMAIN_REORDER = False, KEEP_DUPLICATED = False, \
                    domain_included=False)
    ...
    #domain_subsetbybox(srcdomain_pathfile=DIN_LOC_ROOT+'/share/domains/domain.clm/domain.lnd.r0125_IcoswISC30E3r5.250918.nc', \
    domain_subsetbybox(srcdomain_pathfile=DIN_LOC_ROOT+'/share/domains/domain.clm/domain.lnd.r05_RRSwISC6to18E3r5_N45.240328.nc', \
                          latlons=np.asarray([[63.50,64.00], [-146.00,-145.50]]), \
                          out2D=False, out_type='domain_ncwrite')

    #os.system('mv domain.lnd.r0125_IcoswISC30E3r5.250918.nc_remasked domain.lnd.r0125_IcoswISC30E3r5.250918_remasked.nc')
    os.system('mv domain.lnd.r05_RRSwISC6to18E3r5_N45.240328.nc_remasked domain.lnd.r05_RRSwISC6to18E3r5_N45.240328_remasked.nc')

    allout = refine_surfdata( \
                    lnd_domain_file=DIN_LOC_ROOT+'/share/domains/domain.clm/domain.lnd.r05_RRSwISC6to18E3r5_N45.240328.nc', \
                    fsurdat=DIN_LOC_ROOT+'/lnd/clm2/surfdata_map/surfdata_0.5x0.5_simyr1850_N45_c240308_TOP.nc', \
                    flanduse_timeseries=DIN_LOC_ROOT+'/lnd/clm2/surfdata_map/landuse.timeseries_0.5x0.5_hist_simyr1850-2015_N45_c240308.nc', \
                    #lnd_domain_file=DIN_LOC_ROOT+'/share/domains/domain.clm/domain.lnd.r0125_IcoswISC30E3r5.250918.nc', \
                    #fsurdat=DIN_LOC_ROOT+'/lnd/clm2/surfdata_map/surfdata_0.125x0.125_simyr1850_c250910_TOP.nc', \
                    #flanduse_timeseries=None, \
