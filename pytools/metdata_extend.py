...
if ('cplbypass' in options.met_type or 'cplbypass_site' in options.met_type):
    ...
    ix,iy, varsdims, vardatas = \
        Modules_metdata.clm_metdata_cplbypass_read( \
                            cplbypass_dir, cplbypass_mettype, vnames, [lon], [lat])
...
if ('ATS_h5' in options.user_mettype):
    ...
    vname1=vname_ats
    tunit1 = tunit_ats
elif ('NCDC' in options.user_mettype):
    data1 = sdata_ncdc
    t1 = t_ncdc
    vname1=vname_ncdc
    tunit1 = tunit_ncdc
...
if (BY_JOINTING):
    ...
    if (options.plotting):
        ...
        tx2, datax2, tx2_yrly, datax2_yrly, tx2_seasonally, datax2_seasonally = \
            DataTimePatterns(t_elm, sdata_elm, SEASONALLY, ANNUALLY, ts_yrly=365) 
...
if (options.nc_create or options.nc_write):
    ...
    if (options.nc_create and options.nc_write):
        ...
        sys.exit(-1)
    elif (NCOUT_CPLBYPASS or NCOUT_ORIGINAL):
    ...
    if (not out_metdir.endswith('/')): out_metdir=out_metdir+'/'
    
    in_metdir = options.met_dir
    if (not in_metdir.endswith('/')): in_metdir=in_metdir+'/'
    if 'GSWP3' in out_mettype:
        # non-cplbypass
        if (varname == 'FSDS'):
            fdir = in_metdir+'Solar3Hrly/'
        elif (varname == 'PRECTmms'):
            fdir = in_metdir+'Precip3Hrly/'
        else:
            fdir = in_metdir+'TPHWL3Hrly/'
        fdirheader = in_metdir+'GSWP3_'+varname+'_'
        
        if ('daymet4' in out_mettype.lower()):# from 1980-2014, and 1km resolution
            fdirheader = in_metdir+'GSWP3_daymet4_'+varname+'_'
        elif('daymet' in out_mettype.lower() and 'v1' in out_mettype.lower()):# from 1980-2010, half-degree
            fdirheader = in_metdir+'GSWP3v1_Daymet_'+varname+'_'
        elif('daymet' in out_mettype.lower()):# from 1980-2014, half-degree
            fdirheader = in_metdir+'GSWP3_Daymet_'+varname+'_'
    ...
    if NCOUT_ORIGINAL:
        ...
        if ('GSWP3' in out_mettype): yyyymm = yyyymm.split('_')[-2]
        yyyy=yyyymm.split('-')[-2]
        ...
        for iyr in tyrly:
            ...
            for imm in range(len(mdoy)-1):
                ...
                ncfileout = out_metdir+ncfileout
                #
                if(options.nc_create):
    ...
    if (NCOUT_CPLBYPASS and ncfilein_cplbypass!=''):
        if 'site' in out_mettype.lower()  or 'GSWP3' in out_mettype: 
            if 'site' in out_mettype.lower():
                ...
                sys.exit(-1)
            if(out_metdir+ncfileout_cplbypass==ncfilein_cplbypass and options.nc_create):
                ncfileout_cplbypass = out_metdir+'extended_'+ncfileout_cplbypass
            else:
                ncfileout_cplbypass = out_metdir+ncfileout_cplbypass
            ...
            #
            if error!=0: 
                sys.exit('nfmod.putvar WRONG - '+varname+'-'+ncfileout_cplbypass)
            else:
                print('Sucessfully write - '+varname+' into: '+ncfileout_cplbypass)
