...
#-------------------Parse options-----------------------------------------------
def elmoutput_ncocat(outdir='./', ofheader='none', \
                     varlist='', varlisttxt='', \
                     t_type='time',ncobinpath=''):
    """
        parser.add_option("--outdir", dest="outdir", default="./", \
                  help = 'output directory')
        parser.add_option("--ofheader", dest="ofheader", default="none", \
    ...
    vars_user = [] # this actually will concatentate all variables, except for 'excl_vars'
    if(varlisttxt!=''):  # varlist txt file is checked first, so over-ride 'varlist'
        ftxt = open(varlisttxt,'r')
        vars_user = ftxt.read().splitlines()
        ftxt.close()    
    elif(varlist!=''):
        vars_user = re.split('[:,;]',varlist)
        ...
        if (len(vars_name)==0):
            ...
            for key in allvars:                       
                if(('time' in f.variables[key].dimensions) and (key not in excl_vars)):
                    if(key in vars_user or len(vars_user)==0):
