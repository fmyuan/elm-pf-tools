...
set acme_tag       = v1.0.0-alpha.6
set tag_name       = 20160520
...
set code_root_dir    = /lustre/pfs1/cades-ccsi/proj-shared/models/ACME
set run_root_dir     = /lustre/pfs1/cades-ccsi/scratch
set short_term_archive_root_dir = /lustre/pfs1/cades-ccsi/proj-shared/project_acme/run_archives
set case_root_dir    = /lustre/pfs1/cades-ccsi/proj-shared/project_acme/cases
...
set temp_case_scripts_dir = $case_root_dir/${case_name}    # This part of a workaround to put the case_name into the script names.  It is necessary because create_newcase uses the tail of the case directory.
set case_scripts_dir = $case_root_dir/case_scripts
...
###         rather than embedding the whole env_mach_pes.xml file
  ...
  if ( `lowercase $machine` == 'edison' ) then


    if ( `lowercase $processor_config` == 'custom_m' ) then

    echo 'run_acme: custom_M processor configuration for A_WCYCL* compsets on edison'
...
else if ( $machine == 'cades' ) then
  set input_data_dir = '/lustre/pfs1/cades-ccsi/proj-shared/proj_acme/ACME_inputdata'
endif
...
if ( `lowercase $debug_queue` == 'true' ) then
  ...
  if ( $machine == cab ) then
    ...
    sed -i /"#SBATCH[ ]*--time"/c"#SBATCH  --time=36:00:00"                  ${case_name}.run
  else if ( $machine == titan || $machine == eos  ) then
    sed -i /"#PBS${cime_space}-q"/c"#PBS  -q batch"                          ${case_name}.run
    sed -i /"#PBS${cime_space}-l walltime"/c"#PBS  -l walltime=05:50:00"     ${case_name}.run
  else
...
# Use new river mapping file
./xmlchange -file env_run.xml -id ROF2OCN_RMAPNAME -val 'cpl/cpl6/map_r05_to_oEC60to30_nn.160527.nc'
./xmlchange -file env_run.xml -id ROF2OCN_FMAPNAME -val 'cpl/cpl6/map_r05_to_oEC60to30_nn.160527.nc'
